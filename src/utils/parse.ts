...
export async function parseFile(file: File): Promise<Post[]> {
  ...
  // 检测是否为千帆后台导出格式（第一行是标题，第二行是列名）
  const firstRow = rawRows[0]
  const isQianfanFormat = firstRow.length > 0 &&
    firstRow.every(cell => String(cell).includes('最多导出排序'))

  let rawData: Record<string, unknown>[]

  if (isQianfanFormat && rawRows.length >= 2) {
    // 千帆格式：第一行是标题，第二行是列名，从第三行开始是数据
    // 使用第二行作为 header，从第三行开始读取
    const headers = rawRows[1].map(h => String(h).trim())
    rawData = rawRows.slice(2).map(row => {
  ...
  if (allEmpty) {
    ...
    throw new Error(
      ...
      `【系统期望的列名】\n` +
      `- 标题相关：标题、笔记标题、title\n` +
      `- 曝光相关：曝光量、impressions、展现量、曝光\n` +
      `- 阅读相关：阅读数、阅读量、views、浏览量、观看量\n` +
      `- 点赞相关：点赞数、likes、点赞\n` +
      `- 收藏相关：收藏数、saves、收藏\n` +
      `- 评论相关：评论数、comments、评论\n` +
      `- 分享相关：分享数、转发数、shares、转发、分享\n` +
      `- 涨粉相关：涨粉数、newFollowers、新增粉丝、涨粉`
    )
  }
...
const fieldMapping: Record<string, string[]> = {
  ...
  views: ['阅读数', '阅读量', 'views', '阅读', '浏览', '浏览量', '播放量', '播放', 'read_count', 'view_count', '观看量'],
